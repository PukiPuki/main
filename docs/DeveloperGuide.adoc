...
After forking the repo, links in the documentation will still point to the `se-edu/cardBank-level4` repo. If you plan to develop this as a separate product (i.e. instead of contributing to the `se-edu/cardBank-level4`) , you should replace the URL in the variable `repoURL` in `DeveloperGuide.adoc` and `UserGuide.adoc` with the URL of your fork.
...
[NOTE]
Note how the `Model` simply raises a `CardBankChangedEvent` when the Address Book data are changed, instead of asking the `Storage` to save the updates to the hard disk.
...
Classes used by multiple components are in the `seedu.cardBank.commons` package.
...
**Additional Info:** See our discussion  https://github.com/se-edu/cardBank-level4/issues/390#issuecomment-298936672[here].
